...
| SF-4 (D5c) | **BGR fed to a hardcoded RGB→gray conversion.** The paco layer is decoded `cv2.IMREAD_COLOR` (BGR) and passed to `component_filter._binarize`, which does `COLOR_RGB2GRAY` — R/B luminance weights transposed on every ink pixel. Inverted on the classifier-unreachable fallback path: raw RGB goes to ultralytics, which expects BGR ndarrays. **Measured (MS234_64): minor real contributor — yMAE 0.48 px (max 5.1), grouping distribution shifts 4:7,5:1 → 4:6,5:2, one new line. (Measured on the raw page; interaction with the paco layer still unmeasured, see SF-3.)** | `tasks_predict.py:64` vs `staff-finding/scripts/component_filter.py:470-471`; fallback `tasks_predict.py:121-122` | **fixed 2026-08-17** — paco-layer array converted BGR→RGB immediately after decode (`_decode_paco_layer`, both the fresh and reused-annotation call sites), so every array reaching `infer_staves()`/`component_filter` is consistently RGB. **Ultralytics-side half fixed 2026-08-18** (follow-up on top of this): making every array consistently RGB meant `infer_staves()`/`infer_text_music()`/`infer()`/`infer_staves_raw_boxes()` were now handed RGB unconditionally, but Ultralytics treats a raw array as already BGR (matching its own file-path loader) — so the paco-success path, previously accidentally correct, started feeding YOLO the wrong channel order too. Fixed by centralizing an RGB→BGR conversion (`yolo_inference.py`'s `to_bgr()`) at every YOLO call site, so callers only ever deal in RGB and `YoloModelSet` alone knows about the BGR boundary. Measured via `parity_harness.py` (paco-layer, rgb vs bgr) on `ch-fco-ms2_002r.jpg`: stave detection itself unchanged (77 boxes, 16 staves), centerline-fit accuracy improves 0.57px avg / 7.6px max — fills in the "interaction with the paco layer still unmeasured" gap this row's original measurement (0.48px, on the raw page) had flagged. | 1 |
...
| Cleanup functions run only at process start | `main.py:53-54`, `job_store.py:287,306` (note typo `cleanup_stale_uplaods`) | Celery beat schedule (or equivalent periodic invocation) | **fixed 2026-08-17** — `cleanup_stale_uploads`/`cleanup_stale_sessions` (typo fixed) now also run hourly via the worker's embedded Celery beat (`-B`, `tasks_cleanup.py`); row 28 |
| No `/healthz` on backend/text-service (TCP probes only; paco already has real `/health`+`/ready`) | `k8s/backend.yaml:36-45`, `k8s/text-service.yaml:29,34` | real health endpoints checking DB/broker reachability, wired to probes | **fixed 2026-08-17** — backend gets `/healthz` (readiness, checks Postgres+Celery broker) + `/healthz/live` (liveness, no dependency check, so a transient DB/broker blip doesn't restart-storm the pod); text-service gets a single `/healthz` (no DB/broker of its own); row 29 |
| `BATCH_DIR` + `NEON_MANIFESTS_DIR` node-local disk state | `text-service/main.py:26`, `config.py:18`, `mei_api.py:212-213` | Postgres-backed (like `job_sessions`) or shared storage | **`NEON_MANIFESTS_DIR` half fixed 2026-08-26** — manifests now live in a new `neon_manifests` table (`job_store.neon_manifest_put`/`neon_manifest_get`), written by `mei_api.py`'s `create_edit_session` and served back by a new `GET /neon/samples/manifests/{id}.jsonld` route registered ahead of the old `StaticFiles` mount, at the identical URL shape the `neon/` submodule's `editor.ts` already fetches -- no submodule changes needed. `BATCH_DIR` half tracked separately, spun off from row 30 as [#230](https://github.com/DDMAL/mothra/issues/230) |
| `init_db()`/`_migrate_db()` at import → replicas pinned to 1 | `auth_api.py:253,624`, `k8s/backend.yaml:9` | one-shot migration Job; remove import-time side effect | **fixed 2026-08-17** — `migrate.py` + `k8s/migrate-job.yaml`, wired into `build-images.yml`'s deploy job (apply + `wait --for=condition=complete` before backend/worker); backend/worker's `replicas: 1` is now an operational choice, not a code constraint (worker still has a *different* reason to stay at 1: row 28's embedded Celery beat); row 31 |
| 800 ms sleep hoping Neon's save lands before `PATCH .../corrected` | `NeonBatchEditor.tsx:108` | real completion signal from Neon's `updateDatabase()` | **fixed 2026-08-17** — polls for the real "Saved" toast Notification.ts renders into the iframe's `#notification-content` (bounded, same idiom as `applyNotationTypeFont`'s existing polling) instead of guessing a fixed delay; doesn't touch the `neon/` submodule (its `neonView.save()` Promise isn't exposed past `EditControls.ts`'s private listener -- filing that as a separate DDMAL/Neon enhancement was out of scope for a same-repo PR); row 32 |
| Triplicated inline 86400 s cleanup sweeps at import/request time | `auth_api.py:617-623`, `mei_api.py:115-120`, `text-service/main.py:29-34` | fold into the periodic cleanup above | **fixed 2026-08-17, folded into Celery beat 2026-08-26** — the two Neon-manifest copies (`auth_api.py`, `mei_api.py`) were first consolidated into one shared `auth_api.cleanup_stale_neon_manifests()`, still backend-only since that manifest dir was backend-local disk a worker-run task couldn't reach. Now that manifests live in Postgres ([#230](https://github.com/DDMAL/mothra/issues/230)), `job_store.cleanup_stale_neon_manifests` runs via the same worker Celery-beat sweep as `job_uploads`/`job_sessions`, and the per-request proactive call in `mei_api.py`'s `create_edit_session` is gone. `text-service/main.py`'s batch-zip sweep is unchanged here, tracked separately under [#230](https://github.com/DDMAL/mothra/issues/230)'s `BATCH_DIR` half; row 28 |
...
| # | Task | Notes |
|---|---|---|
| 1.1 | Fix SF-4: convert paco layer BGR→RGB at decode (`tasks_predict.py:64`), and audit every array handoff for channel order (incl. the ultralytics-fallback inversion). Add a channel-order assertion or convention comment at each seam. | Measured first (0.2). **Done 2026-08-18**: decode-side fixed 2026-08-17 (`_decode_paco_layer`); the ultralytics-fallback inversion this task explicitly called out landed as a follow-up (`yolo_inference.py`'s `to_bgr()`, applied at every YOLO call site) — see SF-4's row above for the measured before/after. |
...
|---|---|---|---|---|---|
| DL-1 | `SKIP_PREDICT`/`SKIP_YOLO`/placeholder-bbox-grid triad (`AppRouter.tsx:32-44,287,301-304`, `config.py:30-35`, `ic_api.py:67-120`) — fabricated 6×8 neume grid reaches the real IC | keep as dev-only; gate the grid behind env; remove entirely | Keep the env flags for dev machines, but the fabricated grid must never render without an unmissable "SYNTHETIC — no prediction ran" banner in the IC; alternatively block IC entry without a real annotation row | kyrie | **decided + fixed 2026-08-17** — kept the flags; `ic_api.py`'s `/ic/start` now reports `synthetic: ann_format == "json"`, `InteractiveClassifier.tsx` renders an unmissable yellow banner over the iframe when true. Not reported on a resumed session (IC's staging service doesn't track it after the fact) |
| DL-2 | `ICCompletionTestPage` + hardcoded "ta-da!" copy + `ic-completion` view | rewrite as a real completion view; remove and re-route step 2 | Rewrite: the XML-upload affordance is genuinely useful; the unconditional celebration copy goes | kyrie | **fixed 2026-08-17** — "ta-da!"/"all images successfully classified!" replaced with functional copy ("classification done" / "upload the classifier output to continue to encoding"); upload drop zones unchanged |
| DL-3 | `paco-classifier` pinned to personal branch `gianna/calvo-training-script` (ships production weights) | merge to a stable branch/tag upstream; keep pin with policy | Get calvo work merged upstream to `main`/a tag, then re-pin; until then, freeze the pin (no `--remote` updates) and note it in CODEOWNERS | DDMAL + gianna | **decided 2026-08-17** — freeze noted inline in `.gitmodules` (no CODEOWNERS file exists yet; a full mapping is Phase 6 scope, not created here) |
| DL-4 | ~570 checked-in artifacts | see §5.8 HF-first policy | Triage per §5.8; nothing new to LFS | kyrie | open — spun off, needs its own audit session, not a quick fix |
| DL-5 | eslint disabled in CI (~197 errors) | burn down; baseline + ratchet | Baseline now (fail on *new* errors), burn down as a rolling chore | team | open — spun off, needs its own audit session, not a quick fix |
| DL-6 | `tests.yml` not a required check | make required | Make required as soon as the suite is stable (Phase 5.6) | repo admin | open — GitHub branch-protection setting, not a code change; repo-admin action |
| DL-7 | `run_pageOG.py` + its test + dedicated CI step | delete; keep | Delete once parity harness covers the comparison need; superseded code isn't renamed, it's removed | kyrie | open — needs confirming the golden-page parity CI gate actually covers what `run_pageOG.py`'s comparison was for, not a quick fix |
| DL-8 | Unauth'd `GET /jobs/{id}/stream` (EventSource header limitation; 32-bit ids) | signed short-lived stream token in query; keep with longer ids | Signed one-time token minted at kickoff, passed as query param | kyrie | open — spun off, real auth-design work, not a quick fix |
| DL-9 | `bgr_adapter.py` hardcoded machine paths (BGR fully deferred) | env-var + lazy import; delete | Env-var (`MUSCRAT_LAYER_SEP_DIR`) + lazy import so nothing load-bearing sits on developer paths | kyrie | **decided + fixed 2026-08-17** — hardcoded fallback paths removed, env-var only; the `from bgr_adapter import ...` in `run_page.py` was already lazy (inside `if use_bgr:`), nothing further needed there |
| DL-10 | ProcessingPage cosmetic pacing (log drip-feed, 4 s completion delay) | keep as UX; remove | Remove the 4 s delay; keep at most a short (<1 s) settle animation | kyrie | **fixed 2026-08-17** — `AppRouter.tsx`'s `STEP_TIMING.completionDelayMs` 4000 → 500 |
| DL-11 | "printed text coming soon" disabled radio + backend 400 | keep as roadmap signal; hide | Keep — it's honest roadmap signaling; tidy the copy | kyrie | **fixed 2026-08-17** — kept disabled; relabeled "printed text" → "printed music notation" (was ambiguous with the separate lyric/syllable text-finding step) with a clarifying subtext line |
| DL-12 | Legacy dirs `OLD-annotator/`, root `scripts/`, `configs/` | archive branch; separate research repo; delete | Move still-useful ML scripts to a `research/` area (Phase 6.1); delete the rest; OLD-annotator deleted (tagged release preserves history) | kyrie | open — spun off, needs a per-file usefulness judgment call, not a quick fix |
| DL-13 | Null `confidence` placeholder in JSOMR output (`run_page.py:770`) | compute it; drop the field | Compute from fit quality once defined; until then keep null but document | kyrie | **fixed 2026-08-17** — `run_page.py`'s comment corrected (cited the wrong section, `§5.8`, which is actually the artifact-HF-policy section) and now matches `staffline_stage.py`'s already-correct DL-13 documentation for the same field |
...
| DL-15 | `image_name` uniqueness constraint (2.2/4.6) | constraint + migration; id-only keying everywhere | Both: key by id, add the constraint as a backstop | kyrie | open — spun off, needs a real migration + audit of every id-vs-name keying site, not a quick fix |
| DL-16 | Custom text models in containers (TX-11) | mount volume; ship bytes; out of alpha scope | Out of alpha scope unless a tester needs it; log clearly when a custom path fails to resolve | kyrie | **fixed 2026-08-17** — `text-service/main.py`'s `/run` and `/batch/run` now log an explicit `[warn]` when a custom `segmentation_model`/`recognition_model` path doesn't exist on that container, instead of an opaque downstream Kraken load error |
