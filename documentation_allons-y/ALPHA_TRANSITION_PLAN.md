...
| Triplicated inline 86400 s cleanup sweeps at import/request time | `auth_api.py:617-623`, `mei_api.py:115-120`, `text-service/main.py:29-34` | fold into the periodic cleanup above | **partially fixed 2026-08-17** — the two Neon-manifest copies (`auth_api.py`, `mei_api.py`) consolidated into one shared `auth_api.cleanup_stale_neon_manifests()`; NOT folded into Celery beat, since that manifest dir is backend-local disk a worker-run task can't reach (see its docstring). `text-service/main.py`'s batch-zip sweep is unchanged, deferred to row 30's `BATCH_DIR` migration; row 28 |
