...
| `BATCH_DIR` + `NEON_MANIFESTS_DIR` node-local disk state | `text-service/main.py:26`, `config.py:18`, `mei_api.py:212-213` | Postgres-backed (like `job_sessions`) or shared storage | **`BATCH_DIR` half fixed 2026-08-26** — text-service gained its own Postgres access (`text-service/db.py`); batch zips now live in a new `text_batch_zips` table (schema created by `migrate.py`/`init_db()`, same one-shot migration as everything else) instead of local disk; its retention swept by the worker's existing periodic Celery-beat cleanup instead of the old local-disk TTL. `NEON_MANIFESTS_DIR` half still open, tracked separately — spun off from row 30 as [#230](https://github.com/DDMAL/mothra/issues/230) |
...
| Triplicated inline 86400 s cleanup sweeps at import/request time | `auth_api.py:617-623`, `mei_api.py:115-120`, `text-service/main.py:29-34` | fold into the periodic cleanup above | **partially fixed 2026-08-17, `text-service` half fixed 2026-08-26** — the two Neon-manifest copies (`auth_api.py`, `mei_api.py`) consolidated into one shared `auth_api.cleanup_stale_neon_manifests()`; still NOT folded into Celery beat, since that manifest dir is backend-local disk a worker-run task can't reach (see its docstring) — will fold in once `NEON_MANIFESTS_DIR` itself moves to Postgres ([#230](https://github.com/DDMAL/mothra/issues/230)). `text-service/main.py`'s batch-zip sweep is gone outright: batch zips moved to Postgres (`text_batch_zips`) alongside row 30's `BATCH_DIR` fix, so `job_store.cleanup_stale_batch_zips` now runs via the same Celery-beat schedule as `job_uploads`/`job_sessions`, no local-disk sweep needed; row 28 |
