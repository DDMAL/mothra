...
| 1.10 | Golden-page CI test: harness baseline run on MS234_64 (+ Gent right) as a pytest, tolerances defined, added to `tests.yml`. | Exit gate |
...
| DL-14 | Stave conf default: eval-matched 0.25 vs product 0.5 (SF-1/1.3) | align to 0.25; keep 0.5 and re-evaluate at 0.5 | Align product to whatever the golden-set eval validates; one number, one declaration | kyrie | open |
