...
def test_basic_grouping():
    ...
    for assignment in result.assignments:
        print(
            f"    Fit {assignment.fit_index}: stave_id={assignment.stave_id}, "
    ...
    if actual_stave_ids == expected_stave_ids:
        ...
        print(
            f"  FAIL: Expected stave_ids {expected_stave_ids}, got {actual_stave_ids}"
        )
        return False
...
def test_no_y_values():
    ...
    if result.assignments:
        print(
            f"    Assignment 0: stave_id={result.assignments[0].stave_id}, "
            f"flags={result.assignments[0].flags}"
        )

    if (
        "no_fits_with_y_positions" in result.flags
        and len(result.assignments) == 1
        and result.assignments[0].stave_id is None
    ):
