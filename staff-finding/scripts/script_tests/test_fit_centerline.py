...
def make_synthetic_crop(
    width=400, height=80, line_y=40, line_thickness=4, add_neume=False, empty=False
):
...
def test_clean_line_fit():
    ...
    fit_result = fit_centerline(
        filter_result, scale_unit=4.0, crop=crop, save_path=Path("/tmp/fit_clean.png")
    )
    # Line y is at 40; fit should land near 40 for all sample positions.
    median_y = float(np.median(fit_result.y_values))
    print(
        f"clean line: median fitted y={median_y:.2f}, "
...
def test_sloped_line_fit():
    ...
    filter_result = filter_components(crop, scale_unit=4.0)
    fit_result = fit_centerline(
        filter_result, scale_unit=4.0, crop=crop, save_path=Path("/tmp/fit_sloped.png")
    )
    # Check endpoints land near where the line actually is.
    y_at_start = fit_result.y_values[0]
    y_at_end = fit_result.y_values[-1]
    print(
        f"sloped line: y at x_start={y_at_start:.2f} (expect ~20), "
...
def test_two_line_box_line_following():
    ...
    median_y = float(np.median(fit_result.y_values))
    print(
        f"two-line box: median_y={median_y:.2f}, "
        f"residual_mean={fit_result.residual_mean:.3f}, "
        f"flags={fit_result.flags}"
    )
    assert any(
        "line_following_applied" in f for f in fit_result.flags
