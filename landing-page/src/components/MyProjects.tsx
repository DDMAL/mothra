  ...
  return (
    <div className="animate-fade-in flex-1 bg-[#4AADAA] px-6 py-10">
      ...
      <div className="max-w-4xl mx-auto bg-[#C8E6E3] rounded-2xl overflow-hidden">
        <div className="grid grid-cols-[2fr_2fr_1fr_2.5rem] px-6 py-3 text-[#1D3335] text-sm font-medium border-b border-[#1D3335]/10">
        ...
        {projects.map((p) => (
          <div
            ...
            className="grid grid-cols-[2fr_2fr_1fr_2.5rem] px-6 py-4 border-b border-[#1D3335]/10 last:border-0 items-center text-[#1D3335] text-sm transition-colors hover:bg-[#b0cdc9]"
          >
            <span>{p.name}</span>
            <span>{p.user}</span>
            <span>{p.images}</span>
            <button
              ...
              className={`flex justify-center transition-opacity cursor-pointer text-base ${hoveredRow === p.name ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
            >
              🗑
            </button>
          </div>
      ...
      {showCreate && (
        <div key="create" className="animate-fade-in max-w-lg mx-auto mt-10 bg-[#C8E6E3] rounded-3xl p-8 flex flex-col gap-4 relative">
          <button
