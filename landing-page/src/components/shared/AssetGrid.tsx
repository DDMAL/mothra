...
interface AssetItem { id: string; name: string; }
            ...
            >
                ...
                return (
                    ...
                    <div
                        ...
                        onClick={e => { if (!used) section.handleClick(e, item.id, idx); }}
                    >
                        {renderThumbnail(item)}
                    </div>
                    <div className="flex items-center justify-between gap-1">
                        <span className={`text-sm text-white truncate ${used ? "opacity-40" : ""}`}>{item.name}</span>
                        {!used && (
                        <button
                            onClick={e => {
                            e.stopPropagation();
