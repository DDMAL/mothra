...
export default function MyAccount({
  ...
  const [pendingField, setPendingField] = useState<"username" | "email" | null>(
    null,
  );
  ...
  const usedBytes =
    (usage?.images.bytes ?? 0) +
    (usage?.meiFiles.bytes ?? 0) +
    (usage?.logs.bytes ?? 0);
  const pct = usage?.quotaBytes
    ? Math.min((usedBytes / usage.quotaBytes) * 100, 100)
    : 0;
  ...
  const previousValue =
    pendingField === "username" ? currentUser.username : currentUser.email;

  return (
    <>
      <div className="animate-fade-in flex-1 flex bg-[#4AADAA]">
        <aside
          className="w-72 shrink-0 bg-[#c5dfe0] flex flex-col py-6 px-2
        ...
        <main className="flex-1 overflow-y-auto px-10 py-8">
          <h1 className="text-3xl font-bold italic text-[#1D3335] mb-6">
            my account
          </h1>
          {tab === "info" && (
              ...
              {!usage ? (
                ...
                <>
                  <div className="bg-[#C8E6E3] rounded-3xl p-8 flex flex-col gap-2">
                    <p className="text-[#1D3335] font-semibold text-lg mb-1">
                      total storage
                    </p>
                    <p className="text-4xl font-bold text-[#1D3335]">
                      {formatBytes(
                        usage.images.bytes +
                          usage.meiFiles.bytes +
                          usage.logs.bytes,
                      )}
                    </p>
                    <div className="...">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-[#C8E6E3] rounded-3xl p-6 flex flex-col gap-1">
                      <p className="text-[#1D3335]/60 text-xs uppercase tracking-wide">
                        projects
                      </p>
                      <p className="text-3xl font-bold text-[#1D3335]">
                        {usage.projects.active}
                      </p>
                      <p className="text-sm text-[#1D3335]/70">
                        {usage.projects.deleted > 0
                          ? `+ ${usage.projects.deleted} in trash`
                          : "none in trash"}
                      </p>
                    </div>
                    ...
                    <div className="bg-[#C8E6E3] rounded-3xl p-6 flex flex-col gap-1">
                      ...
                      <p className="text-sm text-[#1D3335]/70">
                        {formatBytes(usage.images.bytes)}
                      </p>
                    </div>
                    ...
                    <div className="bg-[#C8E6E3] rounded-3xl p-6 flex flex-col gap-1">
                      ...
                      <p className="text-3xl font-bold text-[#1D3335]">
                        {usage.meiFiles.count}
                      </p>
                      <p className="text-sm text-[#1D3335]/70">
                        {usage.meiFiles.corrected}/{usage.meiFiles.count}{" "}
                        corrected &nbsp;·&nbsp;
      ...
      {showChangePassword && (
        <Modal size="sm" backdrop="dim" onClose={closePasswordModal}>
          ...
          />
          {pwError && (
            <p className="text-red-600 text-xs text-center">{pwError}</p>
          )}
          <button
            onClick={async () => {
              setPwError("");
      ...
      {showDeleteAccount && (
        ...
        >
          ...
          <p className="text-sm text-[#1D3335] text-center leading-relaxed">
            ...
            <br />
            this is irreversible and you will lose:
          </p>
