...
export default function AnnotationViewerModal({
  ...
  useEffect(() => {
    if (!set.imageSrc) {
      setViewState({
        status: "error",
        message: "No image source for this annotation.",
      });
      return;
    }
    Promise.all([
      apiFetch(`/api/projects/${projectId}/annotations/${set.id}`).then((r) =>
        r.ok ? r.json() : Promise.reject("annotation fetch failed"),
      ),
      apiFetch(set.imageSrc).then((r) =>
      ...
      .catch(() =>
        setViewState({
          status: "error",
          message: "Failed to load annotation view.",
        }),
      );
  }, []);
