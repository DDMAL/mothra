...
export default function TextAlignmentViewerModal({
  alignment,
  projectId,
  onClose,
  label,
  ...
  useEffect(() => {
    if (!alignment.imageSrc) {
      setViewState({
        status: "error",
        message: "No image source for this alignment.",
      });
      return;
    }
    Promise.all([
      apiFetch(
        `/api/projects/${projectId}/text-alignments/${alignment.id}`,
      ).then((r) =>
        r.ok ? r.json() : Promise.reject("alignment fetch failed"),
      ),
      apiFetch(alignment.imageSrc).then((r) =>
      ...
      .then(([data, blob]) => {
        ...
        const parsed = JSON.parse(
          (data as { alignmentJson: string }).alignmentJson,
        );
        setViewState({
          status: "ready",
          imageUrl,
      ...
      .catch(() =>
        setViewState({
          status: "error",
          message: "Failed to load text alignment view.",
        }),
      );
  }, []);
      ...
      <div className="fixed z-50 top-[4.5rem] bottom-4 left-1/2 -translate-x-1/2 w-[calc(100vw-2rem)] max-w-5xl bg-[#C8E6E3] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-fade-in">
        <div className="flex items-center gap-4 px-6 py-3 border-b border-[#1D3335]/20 shrink-0">
          ...
          <span className="text-xs text-[#1D3335]/60">
            {alignment.syllableCount} syllable
            {alignment.syllableCount !== 1 ? "s" : ""}
          </span>
          <button
            onClick={onClose}
              ...
              <div className="flex gap-3 items-start">
                ...
                <div className="shrink-0 flex h-[min(65vh,650px)]">
                  <button
                    ...
                    title={
                      textPanelOpen ? "Hide plain text" : "Show plain text"
                    }
