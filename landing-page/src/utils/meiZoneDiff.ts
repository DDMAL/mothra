...
export function diffZones(originalXml: string, correctedXml: string): ZoneDiff {
  ...
  orig.forEach((origZone, id) => {
    ...
    if (!corrZone) {
      removed.push(origZone);
      return;
    }
    const same =
      origZone.ulx === corrZone.ulx &&
      origZone.uly === corrZone.uly &&
      origZone.lrx === corrZone.lrx &&
      origZone.lry === corrZone.lry;
    if (!same)
      moved.push(corrZone); // show corrected position
