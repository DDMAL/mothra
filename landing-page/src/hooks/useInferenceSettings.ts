...
export function useInferenceSettings() {
  ...
  const [useSharedDetectorSettings, setUseSharedDetectorSettings] = useState(true);
  const [textMusicSettings, setTextMusicSettings] = useState<DetectorSettings>({ threshold: 0.5, device: "cpu" });
  const [staveSettings, setStaveSettings] = useState<DetectorSettings>({ threshold: 0.5, device: "cpu" });
  ...
  const patch = (p: Partial<InferenceSettings>) => {
    ...
    if (p.useSharedDetectorSettings !== undefined) setUseSharedDetectorSettings(p.useSharedDetectorSettings);
    if (p.textMusicSettings !== undefined) setTextMusicSettings(p.textMusicSettings);
    if (p.staveSettings !== undefined) setStaveSettings(p.staveSettings);
  };

  return {
    threshold, device, modelPreset, customModelId,
    useSharedDetectorSettings, textMusicSettings, staveSettings, patch,
  };
}
